...
public class NoTrailingSpacesInChangeLogProcessorTest extends ChangeLogFileRuleProcessorTestUtil {

    private static final RuleEnum RULE_ENUM = RuleEnum.NO_TRAILING_SPACES_IN_CHANGELOG;

    /**
     * Default constructor.
     */
    public NoTrailingSpacesInChangeLogProcessorTest() {
        super(RULE_ENUM);
    }
    ...
    public void testGetName() throws ParserConfigurationException, IOException, SAXException {
        ...
        // act
        RuleEnum actual = NoTrailingSpacesInChangeLogProcessor.instantiate(ruleElement).getName();
    ...
    public void testChangeLogValidationFailure() throws ParserConfigurationException,
        ...
        try {
            ...
            assertEquals("File: [no-trailing-spaces-in-changelog-failure.xml]. "
                    + "Rule [no-trailing-spaces-in-changelog]\n"
                    + "    no-trailing-spaces-in-changelog-failure.xml:9 has trailing spaces or trailing tabs\n"
                    + "    no-trailing-spaces-in-changelog-failure.xml:19 has trailing spaces or trailing tabs\n"
                    + "    no-trailing-spaces-in-changelog-failure.xml:86 has trailing spaces or trailing tabs",
                    e.getMessage());
        }
