...
public class ValidateChangeLogMojoFailureTest {
     ...
     */
    @Test
    public void testExecuteWithoutRulesSourceFailure() throws NoSuchFieldException, IllegalAccessException {
        // arrange
        boolean isExceptionThrown = false;
        setField("pathToRulesFile", null);
        setField("rulesFileUrl", null);
        setField("pathToExclusionsFile", null);
        setField("changeLogDirectory", new File("src/test/resources/db/xml"));
        ...
        try {
            ...
            // assert
            isExceptionThrown = true;
            assertEquals("Exactly one of 'pathToRulesFile' or 'rulesFileUrl' parameters must be present",
                    e.getMessage());
        }
        Assert.assertTrue(isExceptionThrown);
    }
    ...
    /**
     * Integration test for {@link ValidateChangeLogMojo#execute()}.
     * Should fail when both rules file path and URL are provided.
     *
     * @throws NoSuchFieldException   - thrown if required field is missing.
     * @throws IllegalAccessException - thrown if files not found.
     ...
     */
    @Test
    public void testExecuteWithBothRulesSourcesFailure()
    ...
    /**
     * Integration test for {@link ValidateChangeLogMojo#execute()}.
     * Should fail when both exclusions file path and URL are provided.
     *
     * @throws NoSuchFieldException   - thrown if required field is missing.
     * @throws IllegalAccessException - thrown if files not found.
     * @throws MalformedURLException  - thrown if URL is malformed.
     */
    @Test
    public void testExecuteWithBothExclusionsSourcesFailure()
            throws NoSuchFieldException, IllegalAccessException, MalformedURLException {
        // arrange
        boolean isExceptionThrown = false;
        setField("pathToRulesFile", new File("src/test/resources/rules.xml"));
        setField("rulesFileUrl", null);
        setField("pathToExclusionsFile", new File("src/test/resources/exclusions.xml"));
        setField("exclusionsFileUrl", toUrl("src/test/resources/exclusions.xml"));
        setField("changeLogDirectory", new File("src/test/resources/db/xml"));
        ...
        try {
            ...
            // assert
            isExceptionThrown = true;
            assertEquals("Only one of 'pathToExclusionsFile' or 'exclusionsFileUrl' parameters must be present",
                    e.getMessage());
        }
        Assert.assertTrue(isExceptionThrown);
    }
